...
def build_grading_messages(image_base64, standards, streaming=False, image_mime_type='image/png',
    ...
    if streaming:
        ...
        text_parts.append(
            ...
            "- 严禁复述/回显【题目材料】【参考答案】【评价示例】【评分要求】里我给你的任何内容，"
            "它们仅供你评分参考，绝不能出现在你的输出里\n"
...
def make_grading_payload(model_name, image_base64, standards, streaming=False, image_mime_type='image/png',
    ...
    # deepseek-v4-flash 默认输出 reasoning 无 content，需显式关闭推理以保证返回速度
    if provider == 'deepseek':
...
def call_llm_for_grading(provider, api_key, model_name, image_base64, standards,
    ...
    payload = make_grading_payload(model_name, image_base64, standards,
                                   image_mime_type=image_mime_type, ocr_text=ocr_text, provider=provider)
...
def stream_analyze_card_grading(data):
    ...
    worker = threading.Thread(
        ...
        args=(model_provider, model_key, model_call, image_b64, standards, model_queue),
        kwargs={'image_mime_type': image_mime, 'ocr_text': ocr_text or None},
        daemon=True
    )
    worker.start()
    ...
    try:
        ...
        max_score = parsed['max_score']
        reasoning = parsed.get('reasoning', '')
        # 学生作答：OCR 路径以 OCR 文本为准；视觉直评以模型识别文本为准
        student_answer = ocr_text if (ocr_text and ocr_text.strip()) else parsed.get('student_answer', '')
        review_analysis = parsed.get('review_analysis', '')
