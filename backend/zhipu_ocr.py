...
注意：这是**云端**接口（需联网 + 智谱 API Key），不是离线引擎；
离线识别由 local_ocr.py（RapidOCR / PP-OCRv3 ONNX）负责，两者互为降级备份。
"""
...
def call_handwriting_ocr(api_key, image_bytes, mime_type='image/jpeg',
    ...
    """
    if not api_key:
        raise OCRError('请先在AI配置页面填写智谱 API Key', code='ocr_missing_key')
    if not image_bytes:
    ...
    log.info("智谱GLM-OCR 请求: bytes=%d, mime=%s", len(image_bytes), mime_type)
    try:
        resp = requests.post(
            OCR_ENDPOINT,
            headers=headers,
    ...
    if status and str(status).lower() not in ('success', 'ok', 'succeeded', '0'):
    ...
    log.info("智谱GLM-OCR 完成: words=%d, text_len=%d", len(words), len(text))
    return {
        'text': text,
